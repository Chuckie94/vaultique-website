  ...
  A.registerPage({
    ...
    render: function (host, ctx) {
      ...
      function sayWhoIAm() {
        ...
        saidWhoIAm = true;
        Promise.resolve(sb.rpc('chat_staff_me')).then(function (r) {
          var d = r && r.data;
          if (!d) return;
          var can = d.is_admin || d.is_staff;
          var line = el('span', 'lc-whoami',
            (d.display_name ? d.display_name + ' · ' : '') +
            (d.is_owner ? 'shop owner' : d.is_admin ? 'administrator'
                        : d.is_staff ? 'chat agent'
                        : d.known ? 'switched off' : 'no chat access') +
