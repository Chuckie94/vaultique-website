...
(function () {
  ...
  A.registerSetting({
    ...
    render: function (host, ctx) {
      ...
      /* ============== who else signs in ============================
         This card used to be the whole of it: a list of administrators,
         a button to add one, and a second, separate kind of person
         hired to answer chats who signed in somewhere else entirely.

         There is one kind of person now. Everybody signs in here, and
         what they can do comes from their role. That is a bigger job
         than a card on the Security page, so it has a page of its own
         and this points at it. */
      var admCard = el('div', 'card');
      admCard.appendChild(el('h3', null, 'Who else signs in'));
      admCard.appendChild(el('p', 'grp-note',
        'Everybody who works here signs in on this page, and their role decides ' +
        'what they can open. Create them, change a role, reset a password or ' +
        'switch somebody off in Users & Roles.'));
      var admGo = el('button', 'btn btn-out btn-sm', 'Open Users & Roles');
      admGo.type = 'button';
      admGo.addEventListener('click', function () { ctx.navigate('settings', 'users'); });
      admCard.appendChild(admGo);
